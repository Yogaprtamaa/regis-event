...
import { ArrowLeftIcon, CalendarIcon, ClockIcon, MapPinIcon, UsersIcon, PencilSquareIcon, BookOpenIcon, ChatBubbleLeftRightIcon } from "@heroicons/react/24/outline";
...
// QR upload lewat /api/upload/bazaar-qr (service_role)
...
export default function EditEventPage() {
    ...
    const fetchEvent = async () => {
      try {
        ...
        setFormData({
          ...
          isBazaar: !!event.isBazaar,
          isPaidEvent: !!event.isPaidEvent,
          paymentRekening: event.paymentRekening || '',
          paymentBank: event.paymentBank || '',
          paymentAtasNama: event.paymentAtasNama || '',
          paymentQrUrl: event.paymentQrUrl || '',
          paymentNominal: event.paymentNominal || '',
        });
  ...
  const handleChange = (e) => {
    ...
    setFormData(prev => ({
      ...prev,
      [name]: name === 'kapasitas' || name === 'paymentNominal' ? (value === '' ? '' : parseInt(value, 10) || 0) : value,
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if ((formData.isPaidEvent || formData.isBazaar) && !String(formData.paymentRekening).trim()) {
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="bg-[#FDF5E4] b-border rounded-2xl p-4 flex items-center justify-between" style={{ borderColor: formData.isBazaar ? "#F6890C" : "#1a1a1a" }}>
                <div>
                  ...
                  <p className="text-xs text-slate-500">Aktifkan kalau ini booth bazaar</p>
                </div>
              ...
              {(formData.isPaidEvent || formData.isBazaar) && (
                <div className="b-border rounded-2xl p-4 space-y-3" style={{ background: "#fff", boxShadow: "3px 3px 0 #1a1a1a" }}>
                  <p className="text-[11px] font-black uppercase tracking-widest text-slate-600">Info Pembayaran — wajib untuk berbayar / bazaar</p>
