...
// Kebalikan tandatanganiBerkas: form panitia mengirim balik URL yang sudah
// ditandatangani. Kembalikan ke bentuk simpan, kalau tidak DB keisi URL yang
// kedaluwarsa sejam kemudian. Link luar dibiarkan apa adanya.
const PREFIX_TANDA = `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/sign/`;

export function bentukSimpan(url) {
  if (typeof url !== "string" || !url.startsWith(PREFIX_TANDA)) return url;
  return PREFIX + url.slice(PREFIX_TANDA.length).split("?")[0];
}
